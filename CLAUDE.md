...
각 프로젝트의 `CONVENTIONS.md`를 기반으로 코드 컨벤션 위반을 자동으로 탐지하고 수정하는 파이프라인:

```
📅 스케줄 트리거 (매일 오전 9시) 또는 웹훅
    ↓
📋 CONVENTIONS.md 있는 프로젝트 조회
    ↓
...
**사용 방법:**
1. 프로젝트에 `CONVENTIONS.md` 파일 생성 (코딩 규칙 정의)
2. 워크플로우 활성화 또는 웹훅 호출: `POST /webhook/convention-scan`
3. Claude가 자동으로 위반 탐지 및 수정

**API 엔드포인트:**
- `GET /api/v1/convention/{projectId}` - CONVENTIONS.md 조회
